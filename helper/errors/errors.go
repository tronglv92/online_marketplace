...
func (e *errorSvc) Error() string {
	// return fmt.Sprintf("error: code = %d reason = %s", e.Code, e.Reason, e.Metadata, e.cause)
	return fmt.Sprintf("error: code = %d reason = %s metadata=%s cause = %d", e.Code, e.Reason, e.Metadata, e.cause)
}
...
func From(err error) Error {
	if err == nil {
		return InternalServerReason("Internal Server Reason")
	}
